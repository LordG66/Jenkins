...
if [ "${?}" -ne "0" ]; then # branch doesnt exist
    ...
    echo "${version}" > version.txt
    docker build -t lordg66/cowsay:"${version}.${subver}" .
    docker run -p 8000:8000 --name apptest --network jenkins_default -d lordg66/cowsay:"${version}.${subver}" 8000
    while ! wget apptest:8000; do
        sleep 1
    done
    curl apptest:8000
    docker login -u "lordg66" -p "L9o+5xK@b<*q" docker.io
    docker push lordg66/cowsay:"${version}.${subver}"
fi
docker rm -f apptest